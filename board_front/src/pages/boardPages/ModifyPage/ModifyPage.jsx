...
function ModifyPage(props) {
    ...
    const handlemodifySubmitOnClick = () => {
        ...
        instance.put(`/board/modify/${boardInfo.boardId}`, board)
            .then((response) => {
                alert("수정이 완료되었습니다.");
                navigate(`/board/detail/${boardInfo.boardId}`);
            })
            .catch((error) => {
    ...
    const handleQuillValueOnChange = (value) => {
        setBoard(board => ({
            ...board,
            content: quillRef.current.getEditor().getText().trim() !== "" || quillRef.current.getEditor().root.innerHTML ? value : "",
        }));
    }
